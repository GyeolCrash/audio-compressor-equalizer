...
export class EQVisualizer {
  ...
  constructor(private readonly bridge: BackgroundBridge) {
    ...
    this.bridge.send({ type: 'GET_STATUS' });
  ...
  private setupEventListeners(): void {
    ...
    this.canvas.addEventListener('dblclick', (e) => {
      const { x, y } = this.getCanvasCoordinates(e);
      if (!this.getNodeAtPosition(x, y) && this.isInGraphArea(x, y)) this.addNode(x, y);
    });
    ...
    document.addEventListener('keydown', (e) => {
      if (e.key === 'Delete' && this.selectedNodeId !== null) this.removeNode(this.selectedNodeId);
    });

    document.addEventListener('themeChanged', () => this.drawGraph());
  }

  private isInGraphArea(x: number, y: number): boolean {
    return (
      x >= this.padding &&
      x <= this.graphWidth - this.padding &&
      y >= this.padding &&
      y <= this.graphHeight - this.padding
    );
  }
  ...
  addNode(x: number, y: number): void {
    ...
    let id = 0;
    while (this.nodes.some((n) => n.id === id)) id++;
    ...
    this.nodes.sort((a, b) => a.id - b.id);
    this.selectNode(id);
    this.bridge.send({ type: 'ADD_FILTER', nodeId: id, frequency: Math.round(node.frequency) });
  ...
  removeNode(nodeId: number): void {
    ...
    this.bridge.send({ type: 'REMOVE_FILTER', nodeId });
    this.updateNodeList();
    this.updateControlPanel();
    this.drawGraph();
  }
  ...
  deleteSelectedNode(): void {
    if (this.selectedNodeId !== null) this.removeNode(this.selectedNodeId);
  }
  ...
  sendMasterGain(gain: number): void {
    this.bridge.send({ type: 'SET_MASTER_GAIN', gain });
  }
  ...
  private updateControlPanel(): void {
    ...
    if (this.selectedNodeId === null) {
      panel.innerHTML = '<p class="placeholder-text">Select a node</p>';
      return;
    }
    ...
    document.getElementById('freqInput')?.addEventListener('change', (e) => {
      node.frequency = parseFloat((e.target as HTMLInputElement).value);
      this.sendFilterUpdate(node.id);
      this.drawGraph();
    });
    document.getElementById('qInput')?.addEventListener('change', (e) => {
      node.Q = parseFloat((e.target as HTMLInputElement).value);
      this.sendFilterUpdate(node.id);
      this.drawGraph();
    });
    document.getElementById('gainInput')?.addEventListener('change', (e) => {
      node.gain = parseFloat((e.target as HTMLInputElement).value);
      this.sendFilterUpdate(node.id);
      this.drawGraph();
    });
    document.getElementById('filterSelect')?.addEventListener('change', (e) => {
      node.type = (e.target as HTMLSelectElement).value as BiquadFilterType;
      this.sendFilterUpdate(node.id);
      this.drawGraph();
    });
  }
  ...
  private startVisualization(): void {
    ...
    const updateLoop = () => {
      this.bridge.send({ type: 'GET_FREQUENCY_DATA' });
      this.drawGraph();
  ...
  private drawGridLines(styles: CSSStyleDeclaration): void {
    const { ctx, padding, graphWidth, graphHeight } = this;
  ...
  private drawAxisLabels(styles: CSSStyleDeclaration): void {
  ...
  private sendFilterUpdate(nodeId: number): void {
